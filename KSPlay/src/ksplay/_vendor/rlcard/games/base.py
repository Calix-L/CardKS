...
_RANK_ID_BY_VALUE = {
    rank: index for index, rank in enumerate(
        ['A', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K']
    )
}
_SUIT_ID_BY_VALUE = {
    suit: index for index, suit in enumerate(['S', 'H', 'D', 'C', 'BJ', 'RJ'])
}
class Card:
    '''
    ...
    def __hash__(self):
        try:
            return self._gin_rummy_hash
        except AttributeError:
            pass
        try:
            suit_index = _SUIT_ID_BY_VALUE[self.suit]
        except KeyError:
            suit_index = Card.valid_suit.index(self.suit)
        try:
            rank_index = _RANK_ID_BY_VALUE[self.rank]
        except KeyError:
